...
export const getCurrencyRubles = createAsyncThunk<void, undefined, {
    ...
    async (_arg, { dispatch }) => {
        try {
            const { data } = await currencyRublesApi.get<CurrencyRublesTypes.CurrencyRublesType>('');
...
export const getCurrencyChecker = createAsyncThunk<CurrencyChecker.CurrencyChecker, { base_currency: string, second_currency: string }, {
    dispatch: AppDispatch;
    state: State;
}>(
    ...
    async (param) => {
        try {
            const { data } = await currencyCheckerApi.get<CurrencyChecker.CurrencyChecker>(`/pair/${param.base_currency}/${param.second_currency}`);
            return data;
        } catch (error) {
...
export const getTodayCurrency = createAsyncThunk<number, { base_currency: string, second_currency: string }, {
    ...
    async (param) => {
        try {
            ...
            const { data } = await currencyCheckerApi.get<CurrencyChecker.CurrencyHistory>(`/history/${param.base_currency}/${dateString}`);
...
export const getWeatherByCityId = createAsyncThunk<WeatherTypes.WeatherType, number, {
    dispatch: AppDispatch;
    state: State;
}>(
    ...
    async (id) => {
        try {
            const { data } = await weatherApi.get<WeatherTypes.WeatherApiType>('', { params: { id } });
            return {
